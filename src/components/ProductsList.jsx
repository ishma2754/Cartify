        ...
        {products.map((popularProduct) => {
          ...
          return (
            <Link to={`/products/${id}`} key={id} className="block mb-4">
              <div className="flex items-center gap-4 p-4 bg-white shadow-lg rounded-lg hover:shadow-xl transition-shadow duration-300">
                ...
                <div className="ml-4">
                  <p className="text-lg font-semibold text-gray-800">{dollarAmount}</p>
                </div>
              </div>
            </Link>
          );
